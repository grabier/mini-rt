...
/*   Updated: 2025/07/07 15:21:14 by gmontoro         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_parse_camera(char **sp, t_parse *program)
{
	...
	if(!ft_init_cam_origin(program, aux, ft_atof(sp_aux[1]), ft_atof(sp_aux[2])))//check if in the rgb range
		return (printf("camera viewpoint invalid\n"), ft_free(sp_aux), ft_free(sp), 0);
	sp_aux2 = ft_split(sp[2], ',');
	aux = ft_atof(sp_aux2[0]);
	if(!ft_init_cam_dir(program, aux, ft_atof(sp_aux2[1]), ft_atof(sp_aux2[2])))//check if in the rgb range
...
void	ft_init_viewport(t_parse *p)
{
	...
	p->cam->vp_w = p->cam->vp_h * ((float)MAX_W / (float)MAX_H);
	...
	p->cam->lower_left = add(p->cam->center, aux1);
	p->cam->lower_left = add(p->cam->lower_left, aux2);
}
...
int	ft_init_cam_origin(t_parse *program, float x, float y, float z)
{
	...
	program->cam->as_ratio = (float)(MAX_W / MAX_H);
	return (1);
}
